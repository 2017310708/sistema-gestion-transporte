...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="mb-8">
        <h1 class="text-2xl font-semibold text-gray-900">Crear Nueva Ruta</h1>
        <p class="mt-2 text-sm text-gray-700">Complete los detalles para crear una nueva ruta.</p>
    </div>

    <div class="bg-white shadow sm:rounded-lg">
        <form action="{{ route('admin.routes.store') }}" method="POST" class="space-y-6 p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                <div>
                    ...
                    <div class="mt-1">
                        <input type="text" name="origen" id="origen" value="{{ old('origen') }}" required
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="text" name="destino" id="destino" value="{{ old('destino') }}" required
                            class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                    </div>
                    @error('destino')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="datetime-local" name="fecha_salida" id="fecha_salida" value="{{ old('fecha_salida') }}" required
                            class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                    </div>
                    @error('fecha_salida')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input type="datetime-local" name="fecha_llegada" id="fecha_llegada" value="{{ old('fecha_llegada') }}" required
                            class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                    </div>
                    @error('fecha_llegada')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
